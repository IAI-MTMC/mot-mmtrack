...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='FPS to extract images at. Default: 5')
    parser.add_argument(
        '--num-workers',
        type=int,
...
def extract_images(vid_path: str, imgs_dir: str, fps: int):
    ...
    if os.path.exists(imgs_dir):
        ...
        if len(extracted_imgs) == num_frames:
            ...
            print(
                f'Mismatch in number of extracted images. Number of extracted images: {len(extracted_imgs)}. Expected {num_frames}'
            )
        ...
        for scene_dir in tqdm(os.scandir(subset_dir)):
            if scene_dir.is_dir():
                with ThreadPoolExecutor(
                    ...
                    executor.map(extract_images_for_camera,
                                 os.scandir(scene_dir.path))
