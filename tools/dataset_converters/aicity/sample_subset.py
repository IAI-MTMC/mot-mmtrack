...
def parse_args():
    ...
    parser.add_argument(
        '--strategy',
        type=str,
        help="The strategy to sample the subset. Can be 'consec' or 'step'")
...
def main(args):
    ...
    assert args.strategy in (
        'consec', 'step'), "The strategy must be 'consec' or 'step'."
    ...
    for video_id, imgs in image_grs.items():
        assert _check_sorted_by_key(
            imgs, 'frame_id'), 'The images are not in order by frame id'
    ...
    assert _check_sorted_by_key(annotations['images'],
                                'id'), 'The images are not sorted by id.'
    assert _check_sorted_by_key(annotations['annotations'],
                                'id'), 'The annotations are not sorted by id.'

    anns = []
    img_idx, ann_idx = 0, 0
    while img_idx < len(annotations['images']) and ann_idx < len(
    ...
    mmengine.dump(
        ...
        f'{annotation_name}_subset_{args.ratio}_{args.strategy}.json',
    )
