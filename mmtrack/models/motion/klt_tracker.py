...
from .utils import to_tlbr, get_size, get_center, intersection, crop
from .utils import mask_area, transform
...
class KLTTracker:
    def __init__(self, size,
        ...
        if obj_feat_params is not None:
            self.obj_feat_params.update(obj_feat_params)
        if opt_flow_params is None:
            self.opt_flow_params.update(opt_flow_params)
    ...
    def init(self, frame):
        ...
        self.prev_frame_small = cv2.resize(self.prev_frame_gray, self.prev_frame_small.shape[::-1]).astype(np.uint8)
        self.bg_keypoints = np.empty((0, 2), np.float32)
        self.prev_bg_keypoints = np.empty((0, 2), np.float32)

    def predict(self, frame, tracks: List[Track]):
        """Predicts tracklet positions in the next frame and estimates camera motion.

        Parameters
        ----------
        frame : ndarray
            The next frame in BGR mode.
        tracks : List[Track]
        ...
        # self.frame_small = cv2.resize(frame, self.frame_small.shape[::-1])
        self.frame_small = cv2.resize(self.frame_gray, self.frame_small.shape[::-1]).astype(np.uint8)
        ...
        self.prev_frame_bg = cv2.resize(self.prev_frame_gray, self.prev_frame_bg.shape[::-1])
        self.bg_mask_small = cv2.resize(self.fg_mask,
                                        self.bg_mask_small.shape[::-1],
                                        interpolation=cv2.INTER_NEAREST)
        # keypoints = self.bg_feat_detector.detect(self.prev_frame_bg, mask=self.bg_mask_small)
        keypoints = self.bg_feat_detector.detect(cv2.cvtColor(self.prev_frame_bg, cv2.COLOR_GRAY2BGR), mask=self.bg_mask_small)
        if len(keypoints) == 0:
            self.bg_keypoints = np.empty((0, 2), np.float32)
        ...
        for begin, end, track in zip(target_begins, target_ends, tracks):
            ...
            if (intersection(est_tlbr, self.frame_rect) is None or
                ...
                track.keypoints = np.empty((0, 2), np.float32)
                continue
            next_bboxes[track.track_id] = est_tlbr
