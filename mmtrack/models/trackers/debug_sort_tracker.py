...
class SORTTracker(BaseTracker):
    ...
    def track(self,
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            if model.with_motion:
                ...
                for id in self.confirmed_ids:
                    ...
                    kf_boxes.append(torch.from_numpy(self.tracks[id].mean[:4]))
                if len(kf_boxes) > 0:
                    kf_boxes = torch.stack(kf_boxes, dim=0).to(bboxes)
                    kf_boxes = bbox_cxcyah_to_xyxy(kf_boxes)
                    kf_ids = torch.tensor(
                        self.confirmed_ids,
                        dtype=torch.long,
            ...
            if self.with_reid:
                ...
                if len(active_ids) > 0:
                    ...
                    #     num_samples = self.reid.get('num_samples', 1)
                    #     track_embed = []
                    #     for embed, score in zip(self.tracks[id]['embeds'], self.tracks[id]['scores']):
                    #         if score >= 0.7:
                    #             track_embed.append(embed)
                    #     if len(track_embed) == 0:
                    #         track_embed = torch.ones((1, model.reid.head.out_channels)).to(bboxes) * 1e6
                    #     else:
                    #         track_embed = torch.cat(track_embed)[-num_samples:]
                    #         track_embed = torch.mean(track_embed, dim=0, keepdim=True)
                    #     track_embeds.append(track_embed)
                    # track_embeds = torch.cat(track_embeds)
            ...
            if len(active_ids) > 0:
                ...
                if len(active_dets) > 0:
                    ...
                    track_bboxes = torch.stack(track_bboxes).to(bboxes.device)
                    track_bboxes = bbox_cxcyah_to_xyxy(track_bboxes)
                    ious = bbox_overlaps(track_bboxes, bboxes[active_dets])

                    # TODO: support weight_iou_with_det_scores
                    # ious *= scores[active_dets]

                    # support multi-class association
                    track_labels = torch.tensor([
                        self.tracks[id]['labels'][-1] for id in active_ids
                    ]).to(bboxes.device)
                    cate_match = labels[None,
                                        active_dets] == track_labels[:, None]
                    cate_cost = (1 - cate_match.int()) * 1e6
        ...
        # pred_track_instances.bboxes = torch.cat((kf_boxes, bboxes))
        # pred_track_instances.labels = torch.cat((torch.zeros(kf_boxes.size(0)).to(labels), labels))
        # pred_track_instances.scores = torch.cat((torch.zeros(kf_boxes.size(0)).to(scores), scores))
        # pred_track_instances.instances_id = torch.cat((kf_ids, ids))

        try:
            pred_track_instances = InstanceData()
            pred_track_instances.bboxes = torch.cat((kf_boxes, bboxes))
            pred_track_instances.labels = torch.cat(
                (torch.zeros(kf_boxes.size(0)).to(labels), labels))
            pred_track_instances.scores = torch.cat(
                (torch.zeros(kf_boxes.size(0)).to(scores), scores))
            pred_track_instances.instances_id = torch.cat((kf_ids, ids))
        except:
            pred_track_instances = InstanceData()
            pred_track_instances.bboxes = bboxes
