...
class QuasiDenseTrackerPlus(BaseTracker):
    ...
    def __init__(self,
        ...
        self.embed_momentum_factor = memo_momentum
    ...
    def _compute_adaptive_momentum_factor(self, 
                                          conf_score: float,
                                          score_thr: float,
                                          momentum_factor: float) -> float:
        """Compute the adaptive momentum factor to update embedding vector.

        Args:
            conf_score (float): The confidence score.

        Returns:
            float: The adaptive momentum factor.
        """
        alpha = 1 - (conf_score - score_thr) / (1 - score_thr)
        adaptive_momentum_factor = momentum_factor + (
            1 - momentum_factor) * alpha
        adaptive_momentum_factor = min(adaptive_momentum_factor, 1)

        return adaptive_momentum_factor
    ...
    def update(self, ids: Tensor, bboxes: Tensor, embeds: Tensor,
        ...
        for id, bbox, embed, label, score in zip(ids[tracklet_inds],
            ...
            if id in self.tracks.keys():
                ...
                memo_momentum = self._compute_adaptive_momentum_factor(
                    score,
                    self.det_momentum_thr,
                    self.embed_momentum_factor)
                self.tracks[id]['embed'] = (
                    1 - memo_momentum
                ) * self.tracks[id]['embed'] + memo_momentum * embed
    ...
    def track(self,
        ...
        if not hasattr(self, 'motion'):
            self.motion = KLTTracker((img.size(-1), img.size(-2)))
