...
from mmcv.runner.hooks import CheckpointHook, EvalHook, DistEvalHook
...
class DetWandbLoggerHook(WandbLoggerHook):
    """
    Args:
    ...
    def _log_ckpt_as_artifact(self, model_path, aliases, metadata=None):
        ...
        model_artifact = self.wandb.Artifact(
            f"run_{self.wandb.run.id}_model", type="model", metadata=metadata # type: ignore
        )
    ...
    def _add_ground_truth(self, runner):
        ...
        self.class_set = self.wandb.Classes( # type: ignore
        ...
        for idx in self.eval_image_indexs:
            ...
            image = mmcv.bgr2rgb(img_meta["img"]) # type: ignore
    ...
    def _log_predictions(self, results):
        table_idxs = self.data_table_ref.get_index() # type: ignore
        ...
        for ndx, eval_image_index in enumerate(self.eval_image_indexs):
            ...
            if segm_result is not None and len(labels) > 0:
                ...
                segms = mask_util.decode(segms) # type: ignore
            ...
            self.eval_table.add_data(
                self.data_table_ref.data[ndx][0], # type: ignore
                self.data_table_ref.data[ndx][1], # type: ignore
                self.wandb.Image(
                    self.data_table_ref.data[ndx][1], # type: ignore
                    boxes=wandb_boxes,
                    masks=wandb_masks,
    ...
    def _log_data_table(self):
        ...
        """
        data_artifact = self.wandb.Artifact("val", type="dataset") # type: ignore
        data_artifact.add(self.data_table, "val_data")

        if not self.wandb.run.offline: # type: ignore
            self.wandb.run.use_artifact(data_artifact) # type: ignore
            data_artifact.wait()
    ...
    def _log_eval_table(self, idx):
        ...
        pred_artifact = self.wandb.Artifact(
            f"run_{self.wandb.run.id}_pred", type="evaluation" # type: ignore
        )
        ...
        self.wandb.run.log_artifact(pred_artifact, aliases=aliases) # type: ignore
