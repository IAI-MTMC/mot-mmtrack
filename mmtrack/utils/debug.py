...
class ReIDDebugger:
    ...
    def update(self, frame_id: int, img: torch.Tensor, im_mean: List[float], im_std: List[float], embed: torch.Tensor):
    ...
    def visualize(self):
        ...
        for id, info in enumerate(cur_frame):
            ...
            ordered_ids = sorted(list(range(len(prev_frame))), key=lambda x: dists[x])

            imgs = [info["img"]]
            for prev_id in ordered_ids:
                prev_img = prev_frame[prev_id]["img"].copy()
                cv2.putText(prev_img, f"Dist: {dists[prev_id]:.2f}", (0, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                imgs.append(prev_img)
            img = np.concatenate(imgs, axis=1)
            cv2.imwrite(osp.join(self.out_dir, f"{cur_frame_id}_{id}.jpg"), img)
